...
for i, token in enumerate(tokens_reales):
    try:
        ...
        print(f"Error con token {i}: {e}")

dias = ["Lun", "Mar", "Mie", "Jue", "Vie"]
bloques = ["08-10", "10-12", "12-14", "14-16"]
todos_los_bloques = [f"{d}_{b}" for d in dias for b in bloques]
validos = set(todos_los_bloques)

# Reportar bloques inválidos (no coinciden con la lista oficial)
for est in estudiantes:
    for campo in ("v", "v_p", "v_d"):
        if campo in est:
            invalidos = [b for b in est[campo] if b not in validos]
            if invalidos:
                print(f"Estudiante {est.get('n', '?')} tiene bloques inválidos en {campo}: {invalidos}")

pesos_foraneos = np.linspace(1.0, 10.0, 91)
...
# Óptimo real según U_total
idx_opt = int(np.argmax(historico_utotal))
wf_opt = historico_wf[idx_opt]
utotal_opt = historico_utotal[idx_opt]
isn_opt = historico_isn[idx_opt]
bloque_opt = bloques_ganadores[idx_opt]

print(f"Óptimo: W_f = {wf_opt:.1f}, U_total = {utotal_opt:.2f}, ISN = {isn_opt:.1f}%, bloque = {bloque_opt}")
...
plt.axvline(x=wf_opt, color='#A9A9A9', linestyle='--', linewidth=1.5)
ax1.annotate(f'Óptimo: {wf_opt:.1f}\n{bloque_opt}',
             xy=(wf_opt, utotal_opt),
             xytext=(wf_opt + 0.5, utotal_opt),
